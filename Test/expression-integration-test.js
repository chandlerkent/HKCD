...
(new FileList("Test/Files/ExpressionTypeChecker/EarlySamples/*.out").items()).forEach(function(file) {
    var fileBytes = UTILS.readFile(file);
...
// Full
(new FileList("Test/Files/ExpressionTypeChecker/Full/FullTests/*.java").items()).forEach(function(file) {
    files.push(file);
});
(new FileList("Test/Files/ExpressionTypeChecker/Full/ExpectedOutput/*.out").items()).forEach(function(file) {
    var fileBytes = UTILS.readFile(file);
    expected.push((fileBytes === "") ? fileBytes : fileBytes.split("\n"));
});
...
function buildTestFunctionForFile(inFile, errors) {
    ...
    return function() {
        ...
        if (errors.length > 0) {
            ...
            ASSERT.equal(true, found, "Error message: <" + actualMessages.join("\n") + ">\n\ndid not match\n\n<" + errors + ">");
        } else {

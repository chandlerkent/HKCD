...
function checkClassDeclarations(node, env) {
    if (node.isClassDeclaration()) {
        if (env.hasMultipleOfClass(node.getClassName())) {
            env.addError("Multiple declarations found for class " + node.getClassName() + " on line number " + node.getLineNumber() + ".");
        }
        
        if (node.getSuperClassName() && !env.isClassInScopeForClass(node.getSuperClassName(), node.getClassName())) {
            env.addError("Cannot extend the unknown superclass " + node.getSuperClassName() + " on line number " + node.getLineNumber() + ".");
        }
    }
}
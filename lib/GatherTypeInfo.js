...
var gatherTypeInformation = function(node, env) {
    if (node.isClassDeclaration()) {
        if (env.hasClass(node.getClassName())) {
            env.addError("A class named " + node.getClassName() + " has already been defined.");
        } else {
            if (node.hasSuperClass() && !env.hasClass(node.getSuperClassName())) {
                env.addError("Cannot extend an undefined class " + node.getSuperClassName());
            } else {
...
var createMethodType = function(methodDeclNode) {
    ...
    methodType.getParameterTypes = function() {
        var i = this.parameters.length - 1;
        var types = [];
        
        while(i >= 0) {
            types.push(this.parameters[i].type);
            i--;
        }

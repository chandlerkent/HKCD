...
var walkAndProcessAST = function(node, env) {
    ...
    if (node.isInitialAssignment()) {
        ...
        if (env.isFunctionType(node.getValue().getExpressionType(env))) {
            var closure = new MethodMap(node.getVariableName(), node.getValue().getReturnExpression());
            node.getValue().children.forEach(function(child) {
                if (child.isParameterDeclaration()) {
                    closure.addParameter(new VariableMap(child.getParameterName(), child.getType()));
                }
            });
            var variable = new VariableMap(node.getVariableName(), node.getType());

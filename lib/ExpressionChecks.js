...
exports.Subtract = function(aType, env) {
    ...
    if(!this.children[0].checkType("int", env)) {
        env.addError(new TypeError("The left side of the subtract expression is not an integer!", this.getLineNumber()));
        valid = false;
    }
        
    if(!this.children[1].checkType("int", env)) {
        env.addError(new TypeError("The right side of the subtract expression is not an integer!", this.getLineNumber()));
        valid = false;
    }
        
    if(aType !== "int") {
        env.addError(new TypeError("A subtract expression does not return " + aType, this.getLineNumber()));
        valid = false;
    }

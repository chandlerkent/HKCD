...
Environment.prototype.isFunctionType = function(aType) {
...
Environment.prototype.classHasField = function(aClass, fieldName) {
    if(!this.hasClass(aClass))
        return false;
    
    if(this.getClass(aClass).hasField(fieldName))
        return true;

    if(!this.getClass(aClass).getSuperclass())
        return false;
        
    return this.classHasField(this.getClass(aClass).getSuperclass(), fieldName);
};
...
Environment.prototype.getFieldForClass = function(aClass, fieldName) {
    if(!this.hasClass(aClass))
        return null;
    
    if(this.getClass(aClass).hasField(fieldName)) {
        return this.getClass(aClass).getField(fieldName);
    }
    ...
    return this.getFieldForClass(this.getClass(aClass).getSuperclass(), fieldName);
};
...
Environment.prototype.isTypeOrSuperType = function(typeToCheck, typeToFind) {
    if(typeToCheck === typeToFind)
        return true;
        
    if(!this.hasClass(typeToFind))
        return false;
        
    if(!this.getClass(typeToFind).superclass)
        return false;
        
    return this.isTypeOrSuperType(typeToCheck, this.getClass(typeToFind).superclass);
};
...
Environment.prototype.getInterfaceTypeOfClosure = function(params, returnType) {
    var key = params.join(",") + "=>" + returnType;
    
    if (!this.closureTypeMap[key])
        this.closureTypeMap[key] = "Function$" + this.closureCounter++;

    return this.closureTypeMap[key];
};

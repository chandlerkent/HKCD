...
exports.Closure = function(aType, env) {
    if (!simpleCheck("Function", aType, "The type of a closure was expected to be " + aType + " but is really a Function", this.getLineNumber(), env)) {
        return false;
    }
    
    var errorCount = env.errors.length;

    this.children.forEach(function(child) {
        if (child.isParameterDeclaration()) {
            env.getCurrentScope().addVariable(new VariableMap(child.getParameterName(), child.getType()));
        }
    });

    var tc = new TypeChecker.TypeChecker([TypeChecker.IfWhile, TypeChecker.PrintLine, TypeChecker.Assignment, TypeChecker.InitialAssignment]);
    tc.typeCheck(this, env);
    
    if (env.errors.length != errorCount) {
        env.addError(new TypeError("The body of a closure has type errors.", this.getLineNumber()));
        return false;
    }

    this.getReturnExpression().setScope(env.getCurrentScope().copy());
    return this.getReturnExpression().expression.checkType(this.getReturnExpression().expression.getExpressionType(env), env);
};
...
exports.ClosureCall = function(aType, env) {
    ...
    var closure = env.getCurrentScope().getVariable(closureName).getValue();
    
    var expectedMethodParams = closure.getParameters();
    var actualMethodParamExpr = this.children[0];

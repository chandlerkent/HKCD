...
exports.Construct = function(aType, env) {
    if(simpleCheck(aType, this.constructor, "Attempting to return " +
            this.constructor + " but expecting " + aType, this.getLineNumber(), env)) {
        
            if(!env.hasClass(this.constructor)) {
                env.addError(new TypeError("Cannot instantiate the type " + this.constructor + 
                    " because it was never defined.", this.getLineNumber()));
                return false;
            }
...
exports.ID = function(aType, env) {
    ...
    return simpleCheck(aType, actualType, "Expected variable " + this.value + " to be of type " + aType + ", but was of type " + actualType, this.getLineNumber(), env);
};
...
exports.Call = function(aType, env) {
    ...
    if (!classType) {
        env.addError(new TypeError("No variable named " + this.children[0].getValue() + " in the current scope"));
        return false;
    }
    ...
    if(!env.classHasMethod(classType, methodCallName)) {
        env.addError(new TypeError("The class " + classType + " does not have the method " + methodCallName + ".", 
            this.getLineNumber()));
        return false;
    }
    
    var actualType = env.returnTypeOfMethodForClass(classType, methodCallName);
    return simpleCheck(aType, actualType, "The return type of the method " + methodCallName + 
        " was expected to be " + aType + " but is of type " + actualType, this.getLineNumber(), env);
};

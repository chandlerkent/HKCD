...
exports.ClassDecl = function(emitter, env) {
    ...
    emitter.emitClassInitializer(this.getSuperClassName());
...
exports.ReturnExpression = function(emitter, env) {
    ...
    emitFunctionForType(this.getExpression().getExpressionType(env), env, {
...
exports.Assignment = function(emitter, env) {
    ...
    var curClass = env.getCurrentClass();
    var varName = this.getVariableName();
    
    if (curClass.hasField(varName)) {
        var type = curClass.getField(varName).getType();
        emitter.emitObjectLoad("this");
        this.getValue().emit(emitter, env);
        emitter.emitPutField(curClass.getKey(), varName, type);
    } else {
        this.getValue().emit(emitter, env);
        emitFunctionForType(this.getValue().getExpressionType(env), env, {
            "int":function(){emitter.emitExistingIntegerStore(emitter.getLocalVariable(varName));},
            "object":function(){emitter.emitExistingObjectStore(varName);}
        });
    }
...
exports.IDExpression = function(emitter, env) {
    ...
    if (curClass.hasField(value)) {
        var type = curClass.getField(value).getType();
        emitter.emitObjectLoad("this");
        emitter.emitGetField(curClass.getKey(), value, type);
    } else {
